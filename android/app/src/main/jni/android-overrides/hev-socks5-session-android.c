/*
 * Android socks5 session override.
 *
 * The upstream implementation supports socket marks. Catch Report currently
 * keeps the app UID outside its own VPN with VpnService.Builder, so the native
 * session does not call VpnService.protect(fd) from HEV worker threads.
 */
